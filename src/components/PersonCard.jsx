...
const PersonCard = ({ person, onClick }) => {
  ...
  const alias = person.alias || person.remark;
  const festivals = person.festivals || person.important_days || [];
  const lastInteraction = person.lastInteraction || person.created_at?.split('T')[0];
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        <div className="w-16 h-16 bg-[#fcd753] rounded-full flex items-center justify-center shadow-md overflow-hidden">
          {person.avatar_url ? (
            <img
        ...
        <div className="flex-1">
          ...
          {alias && (
            <p className="text-sm text-[#897dbf] mb-1">{alias}</p>
          )}

          <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
            <Calendar className="h-4 w-4 text-[#897dbf]" />
            <span>最近互动: {lastInteraction}</span>
          </div>
